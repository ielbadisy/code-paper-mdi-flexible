...
TVE_3030 %>%
  ...
  stat_function(fun=function(x) {exp(0.01+(1.2/exp(x-0.02*x^0.7)))})+
  coord_cartesian(ylim=c(0, 5), expand = TRUE)+
  labs( x = "Time (years)",
        y = expression(HR[X[3]]))+  
  theme(plot.title = element_text(hjust = 0.5))+
  theme_bw()
ggsave("TVE_MCAR_decreasing.png", width = 8, height = 6, dpi = 300)
...
TVE_3030 %>%
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  theme_bw()
ggsave("TVE_MAR_decreasing.png", width = 8, height = 6, dpi = 300)
...
TVE_3030 %>%
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  theme_bw()
ggsave("TVE_MNAR_decreasing.png", width = 8, height = 6, dpi = 300)
