...
TVE_3030 %>%
  ...
  stat_function(fun=function(x) {exp(0.01+(0.65*x^0.4))})+ 
  coord_cartesian(ylim=c(0, 5), expand = TRUE)+
  labs(  x = "Time (years)",
         y = expression(HR[X[3]]))+  
  theme(plot.title = element_text(hjust = 0.5))+
  theme_bw()
ggsave("TVE_MCAR_increasing.png", width = 8, height = 6, dpi = 300)
...
TVE_3030 %>%
  ...
  coord_cartesian(ylim=c(0, 5), expand = TRUE)+
  labs(  x = "Time (years)",
         y = expression(HR[X[3]]))+  # , caption = ""
  theme(plot.title = element_text(hjust = 0.5))+
  theme_bw()
ggsave("TVE_MAR_increasing.png", width = 8, height = 6, dpi = 300)
...
TVE_3030 %>%
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  theme_bw()
ggsave("TVE_MNAR_increasing.png", width = 8, height = 6, dpi = 300)
